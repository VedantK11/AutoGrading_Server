...
// EMISSARY implementation
// instantiating with priority bit
CACHE::tag_lookup_type::tag_lookup_type(request_type req, bool local_pref, bool skip)
    : address(req.address), v_address(req.v_address), data(req.data), ip(req.ip), instr_id(req.instr_id), pf_metadata(req.pf_metadata), cpu(req.cpu),
      ...
      emissary_priority(req.emissary_priority), is_data(req.is_data)
{
}
...
CACHE::mshr_type CACHE::mshr_type::merge(mshr_type predecessor, mshr_type successor)
{
  ...
  // DATA-INSTR implementation
  retval.is_data = predecessor.is_data;
...
// EMISSARY implementation
// changed intialization to include priority
CACHE::BLOCK::BLOCK(mshr_type mshr)
    : valid(true), prefetch(mshr.prefetch_from_this), dirty(mshr.type == access_type::WRITE), address(mshr.address), v_address(mshr.v_address), data(mshr.data),
      emissary_priority(mshr.emissary_priority), is_data(mshr.is_data)
{
}

bool CACHE::handle_fill(mshr_type& fill_mshr)
{
  if (NAME == "cpu0_L1D" || NAME == "cpu0_DTLB") {
    fill_mshr.is_data = true;
  } else if (NAME == "cpu0_L1I" || NAME == "cpu0_ITLB") {
    fill_mshr.is_data = false;
  }
  ...
  auto way = std::find_if_not(set_begin, set_end, [NAME = this->NAME](auto& x) {
    if (NAME == "cpu0_L1D" || NAME == "cpu0_DTLB") {
      x.is_data = true;
    } else if (NAME == "cpu0_L1I" || NAME == "cpu0_ITLB") {
      x.is_data = false;
  ...
  if (way != set_end) {
    if (way->valid && way->dirty) {
      ...
      // EMISSARY implementation
      writeback_packet.emissary_priority = way->emissary_priority; // copying priority to writeback packet

      // DATA-INSTR implementation
      writeback_packet.is_data = way->is_data;

      if (NAME == "cpu0_L2C") {
        writeback_packet.emissary_priority = false;
      }
  ...
  if (success) {
    ...
    response_type response{fill_mshr.address,           fill_mshr.v_address, fill_mshr.data, metadata_thru, fill_mshr.instr_depend_on_me,
                           fill_mshr.emissary_priority, fill_mshr.is_data};
...
{
  if (NAME == "cpu0_L1D" || NAME == "cpu0_DTLB") {
    handle_pkt.is_data = true;
  } else if (NAME == "cpu0_L1I" || NAME == "cpu0_ITLB") {
    handle_pkt.is_data = false;
  }
  ...
  if (should_activate_prefetcher(handle_pkt)) {
    ...
    // DATA-INSTR implementation
    prefetcher_is_data = handle_pkt.is_data;
  ...
  if (hit) {
    ...
    if (handle_pkt.is_data) {
      ++sim_stats.data_hits[champsim::to_underlying(handle_pkt.type)][handle_pkt.cpu];
    } else {
      ++sim_stats.instr_hits[champsim::to_underlying(handle_pkt.type)][handle_pkt.cpu];
    }
    ...
    response_type response{handle_pkt.address,           handle_pkt.v_address, way->data, metadata_thru, handle_pkt.instr_depend_on_me,
                           handle_pkt.emissary_priority, handle_pkt.is_data};
    ...
    // EMISSARY implementation
    way->emissary_priority = handle_pkt.emissary_priority;

    // DATA-INSTR implementation
    way->is_data = handle_pkt.is_data;

    if (NAME == "LLC") {
      way->emissary_priority = false;
    }
...
bool CACHE::handle_miss(tag_lookup_type& handle_pkt)
{
  ...
  // check mshr
  auto mshr_entry =
      std::find_if(std::begin(MSHR), std::end(MSHR), [match = handle_pkt.address >> OFFSET_BITS, shamt = OFFSET_BITS, NAME = this->NAME](auto& entry) {
        if (NAME == "cpu0_L1D" || NAME == "cpu0_DTLB") {
          entry.is_data = true;
        } else if (NAME == "cpu0_L1I" || NAME == "cpu0_ITLB") {
          entry.is_data = false;
  ...
  if (mshr_entry != MSHR.end()) // miss already inflight
  {
    ...
    *mshr_entry = mshr_type::merge(*mshr_entry, to_allocate);
    ...
    // EMISSARY implementation
    fwd_pkt.emissary_priority = handle_pkt.emissary_priority; // copying priority

    // DATA-INSTR implementation
    fwd_pkt.is_data = handle_pkt.is_data;

    if (NAME == "LLC") {
      fwd_pkt.emissary_priority = false;
    }
  ...
  if (handle_pkt.is_data) {
    ++sim_stats.data_misses[champsim::to_underlying(handle_pkt.type)][handle_pkt.cpu];
  } else {
    ++sim_stats.instr_misses[champsim::to_underlying(handle_pkt.type)][handle_pkt.cpu];
  }
...
long CACHE::operate()
{
  ...
  // Finish returns
  std::for_each(std::begin(lower_level->returned), std::end(lower_level->returned), [this](auto& pkt) {
    if (this->NAME == "cpu0_L1D" || this->NAME == "cpu0_DTLB") {
      pkt.is_data = true;
    } else if (this->NAME == "cpu0_L1I" || this->NAME == "cpu0_ITLB") {
      pkt.is_data = false;
    }
  ...
  if (lower_translate != nullptr) {
    std::for_each(std::begin(lower_translate->returned), std::end(lower_translate->returned), [this](auto& pkt) {
      if (this->NAME == "cpu0_L1D" || this->NAME == "cpu0_DTLB") {
        pkt.is_data = true;
      } else if (this->NAME == "cpu0_L1I" || this->NAME == "cpu0_ITLB") {
        pkt.is_data = false;
  ...
  for (auto q : {std::ref(MSHR), std::ref(inflight_writes)}) {
    auto [fill_begin, fill_end] = champsim::get_span_p(std::begin(q.get()), std::end(q.get()), fill_bw, [cycle = current_cycle, NAME = this->NAME](auto& x) {
      if (NAME == "cpu0_L1D" || NAME == "cpu0_DTLB") {
        x.is_data = true;
      } else if (NAME == "cpu0_L1I" || NAME == "cpu0_ITLB") {
        x.is_data = false;
    ...
    auto complete_end = std::find_if_not(fill_begin, fill_end, [this](auto& x) { return this->handle_fill(x); });
  ...
  auto [last_not_missed, stash_end] = champsim::extract_if(std::begin(inflight_tag_check), std::end(inflight_tag_check), std::back_inserter(translation_stash),
                                                           [cycle = current_cycle, NAME = this->NAME](auto& x) {
                                                             if (NAME == "cpu0_L1D" || NAME == "cpu0_DTLB") {
                                                               x.is_data = true;
                                                             } else if (NAME == "cpu0_L1I" || NAME == "cpu0_ITLB") {
                                                               x.is_data = false;
                                                             }
                                                             return x.event_cycle < cycle && !x.is_translated;
                                                           });
  ...
  auto [tag_check_ready_begin, tag_check_ready_end] =
      champsim::get_span_p(std::begin(inflight_tag_check), std::end(inflight_tag_check), MAX_TAG, [cycle = current_cycle, NAME = this->NAME](auto& pkt) {
        if (NAME == "cpu0_L1D" || NAME == "cpu0_DTLB") {
          pkt.is_data = true;
        } else if (NAME == "cpu0_L1I" || NAME == "cpu0_ITLB") {
          pkt.is_data = false;
...
int CACHE::prefetch_line(uint64_t pf_addr, bool fill_this_level, uint32_t prefetch_metadata)
{
  ...
  pf_packet.is_data = prefetcher_is_data;
...
void CACHE::finish_packet(const response_type& packet)
{
  ...
  // EMISSARY implementation
  mshr_entry->emissary_priority = packet.emissary_priority;

  // DATA-INSTR implementation
  mshr_entry->is_data = packet.is_data;
...
void CACHE::finish_translation(const response_type& packet)
{
  ...
  auto mark_translated = [is_data = packet.is_data, p_page = packet.data, this](auto& entry) {
...
void CACHE::issue_translation()
{
  auto issue = [this](auto& q_entry) {
    if (!q_entry.translate_issued && !q_entry.is_translated) {
      ...
      // EMISSARY implementation
      fwd_pkt.emissary_priority = q_entry.emissary_priority;

      // DATA-INSTR implementation
      fwd_pkt.is_data = q_entry.is_data;
...
void CACHE::end_phase(unsigned finished_cpu)
{
  for (auto type : {access_type::LOAD, access_type::RFO, access_type::PREFETCH, access_type::WRITE, access_type::TRANSLATION}) {
    ...
    roi_stats.data_hits.at(champsim::to_underlying(type)).at(finished_cpu) = sim_stats.data_hits.at(champsim::to_underlying(type)).at(finished_cpu);
    roi_stats.data_misses.at(champsim::to_underlying(type)).at(finished_cpu) = sim_stats.data_misses.at(champsim::to_underlying(type)).at(finished_cpu);
    roi_stats.instr_hits.at(champsim::to_underlying(type)).at(finished_cpu) = sim_stats.instr_hits.at(champsim::to_underlying(type)).at(finished_cpu);
    roi_stats.instr_misses.at(champsim::to_underlying(type)).at(finished_cpu) = sim_stats.instr_misses.at(champsim::to_underlying(type)).at(finished_cpu);
  }

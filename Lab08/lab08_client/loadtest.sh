...
for ((i = 1; i <= numClients; i++)); do
    ./gradingclient "$Server_Ip:$portNum" pass.cpp "$loopNum" "$sleepTimeSeconds" 40 > "$outputDir/client_$i.txt" &
...
for ((i = 1; i <= numClients; i++)); do
    ...
    timeoutRequests=$(($timeoutRequests + $(grep "Timeout Requests: " "$output_file" | awk '{print $NF}')))
    errorRequests=$(($errorRequests + $(grep "Error Requests: " "$output_file" | awk '{print $NF}')))
...
timeoutRequestsRate=$(echo "scale=2; $timeoutRequests / $totalRequests" | bc)
errorRequestsRate=$(echo "scale=2; $errorRequests / $totalRequests" | bc)
successfulRequestsRate=$(echo "scale=2; $totalThroughput / $numClients" | bc)
RequestsSentRate=$(echo "scale=2; $totalThroughput +$timeoutRequestsRate +$errorRequestsRate" | bc)
...
echo "Successful Requests Rate(Goodput): $successfulRequestsRate"
echo "Timeout Requests Rate: $timeoutRequestsRate"
echo "Error Requests Rate: $errorRequestsRate"

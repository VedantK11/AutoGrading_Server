...
for num_clients in num_clients_list:
    ...
    successful_requests_rate = float(output_lines[1].split(': ')[1].split()[0])
    timeout_requests_rate = float(output_lines[2].split(': ')[1])
    error_requests_rate = float(output_lines[3].split(': ')[1])
    requests_sent_rate = float(output_lines[4].split(': ')[1])
    average_active_threads = float(output_lines[5].split(': ')[1])
    overall_cpu_utilization = float(output_lines[6].split(': ')[1].rstrip('%'))
